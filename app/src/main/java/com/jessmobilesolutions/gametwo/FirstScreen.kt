...
class FirstScreen(val game: Game) : Screen(game) {
    ...
    override fun draw() {
        ...
        canvas.drawText("Jogo Teste", canvas.width/2f, 120f, textPaint)
        textPaint.textSize = 50f
        canvas.drawText("(Toque no quadrado para entrar...)", canvas.width/2f, 200f, textPaint)
    }

    override fun handleEvent(event: Int, x: Float, y: Float) {
        if (event == MotionEvent.ACTION_DOWN) {
            if (boundingBox.contains(x.toInt(), y.toInt())) {
                game.actualScreen = SpiderScreen(game)
            }
        }
    }
